...
{	
  ...
  if ( card == adventurer )
    ...
    {
      if (refactorCouncilRoom (state, handPos) < 0)
          {
            return -1;	
          }
    }
    ...
    {  
      if (refactorFeast (choice1, state) < 0)
          {
            return -1;  
          } 
    }
    ...
    {
      if (refactorSmithy (state, handPos) < 0)
          {
            return -1;
          }
    }
    ...
    {
      if (refactorMine(choice1, choice2, state, handPos) < 0)
          {
            return -1;
          }
    }
  else if ( cardEffect(card, choice1, choice2, choice3, state, handPos, &coin_bonus) < 0 )
    {
      return -1;
    }
...
int refactorAdventurer(struct gameState *state){
      int z = 0;
      int drawntreasure = 1;
      int currentPlayer = whoseTurn(state);
...
void refactorCouncilRoom(struct gameState *state, int handPos)
{
      int i; 
      int currentPlayer = whoseTurn(state);
...
int refactorFeast(int choice1, struct gameState *state)
{
      int i;
      int x;
      int currentPlayer = whoseTurn(state);
...
int refactorSmithy(struct gameState *state, int handPos)
{
      int i;
      int currentPlayer = whoseTurn(state);
...
int refactorMine(int choice1, int choice2, struct gameState *state, int handPos)
{
      int i;
      int j;
      int currentPlayer = whoseTurn(state);
